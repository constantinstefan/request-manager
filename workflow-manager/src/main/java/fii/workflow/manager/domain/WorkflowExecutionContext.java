...
public class WorkflowExecutionContext {
    ...
    public Map<String, byte[]> getFiles(String fileVariables) {
        if(fileVariables == null || fileVariables.isEmpty()) return Collections.emptyMap();
        List<String> fileVariablesList = Arrays.asList(fileVariables.split(";"));
        fileVariablesList.forEach(fileVariable -> {
            if(! fileVariable.startsWith("$")) {
                throw new NoSuchElementException("Invalid file variable");
            }
        });
